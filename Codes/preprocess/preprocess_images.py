...
if __name__ == '__main__':
    ...
    trainLabels = pd.read_csv(root_dir+'labels/trainLabels.csv')

    trainLabels['image'] = [i + '.jpeg' for i in trainLabels['image']]
    trainLabels['black'] = np.nan

    trainLabels['black'] = find_black_images(root_dir+'data/train-resized-256/', trainLabels)
    trainLabels = trainLabels.loc[trainLabels['black'] == 0]


    # store to file master 
    trainLabels.to_csv(root_dir+'labels/trainLabels_master.csv', index=False, header=True)
